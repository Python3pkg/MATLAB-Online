...
if ~strcmp(obj.data{patchIndex}.type, 'mesh3d')
    ...
    % handle vertices
    x_data = patch_data(1).Vertices(:,1);
    y_data = patch_data(1).Vertices(:,2);
    z_data = patch_data(1).Vertices(:,3);

    % specify how vertices connect to form the faces
    i_data = patch_data(1).Faces(:,1)-1; 
    j_data = patch_data(1).Faces(:,2)-1;
    k_data = patch_data(1).Faces(:,3)-1;
